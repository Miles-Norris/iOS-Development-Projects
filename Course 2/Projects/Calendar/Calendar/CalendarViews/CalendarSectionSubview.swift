    ...
    var body: some View {
        if !section.isEmpty {
            ...
            ForEach(section) { lesson in
                NavigationLink(destination: CalendarEntryDetailView(currentEntry: lesson, currentUser: currentUser, viewModel: CalendarEntryDetailViewModel(currentUser: currentUser), assignmentStore: $assignmentStore, calendarStore: $calendarStore)) {
