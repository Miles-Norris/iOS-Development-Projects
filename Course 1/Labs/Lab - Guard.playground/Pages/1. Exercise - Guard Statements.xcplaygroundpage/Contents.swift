 ...
 */
func calcutaleArea(x: Double, y: Double) -> Double? {
    guard x > 0 && y > 0 else {
        return nil
    }
    return x * y
...
if let area1 = calcutaleArea(x: 63, y: 44) {
    print(area1)
}
if let area2 = calcutaleArea(x: 63, y: -44) {
    print(area2)
}
...
func add(x: Int?, y: Int?) -> Int? {
    guard let xUnwrapped = x,
          let yUnwrapped = y else {
        return nil
    }
    return xUnwrapped + yUnwrapped
}

if let sum1 = add(x: 3, y: 34) {
    print(sum1)
}
if let sum2 = add(x: nil, y: 78) {
    print(sum2)
}
/*:
...
func createUser() -> User? {
    ...
    let createdUser = User(firstName: firstName, lastName: lastName, age: age)
    return createdUser
}
...
if let user1 = createUser() {
    print("The users name is \(user1.firstName) \(user1.lastName), and they are \(user1.age) years old.")
}
