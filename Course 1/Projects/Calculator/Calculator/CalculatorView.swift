            ...
            VStack {
                ...
                LazyVGrid(columns: [GridItem(), GridItem(), GridItem(), GridItem()], spacing: 7) {
                    ForEach(calculatorButtons) { button in
                        CalculatorButtonSubView(viewModel: $viewModel, buttonClosure: button.buttonClosure, buttonWidth: isPortrait ? proxy.size.width / 4 - 12 : proxy.size.width / 4 - 75, buttonHeight: isPortrait ? proxy.size.height / 15 - 3 : proxy.size.height / 11, buttonLabelSize: button.buttonLabelSize, buttonLabel: button.buttonLabel, isSystemImage: button.isSystemImage)
                    }
                }
            }
